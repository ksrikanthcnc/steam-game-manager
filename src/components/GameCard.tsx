...
function getScreenshots(game: GameWithTags): string[] {
  ...
  const count = game.total_screenshots || 0;
  if (count === 0) return [];
  return Array.from({ length: count }, (_, i) => `/api/assets/${aid}/ss_${i}.jpg`);
}
...
export default function GameCard({ game, selected, slideshow, slideDelay = 1000, pageFocused = true, defaultImage = "header", genresCount = 3, communityTagsCount = 4, onClick, onTagInclude, onTagExclude, onSubtagInclude, onSubtagExclude, onGenreFilter, onCommunityTagFilter, colorCoded, scoreSource = "steamdb", tintColors }: Props) {
  ...
  const aid = assetId(game);
  const headerImg = defaultImage === "header"
    ? `/api/assets/${aid}/header.jpg`
    : `/api/assets/${aid}/${defaultImage}.jpg`;
  const genres = safeJsonParse(game.steam_genres).slice(0, genresCount);
  const screenshots = getScreenshots(game);

  // Reset image state when the image URL or game data changes (e.g. after metadata sync)
  const prevImgRef = useRef(headerImg);
  const prevUpdatedRef = useRef(game.updated_at);
  useEffect(() => {
    if (prevImgRef.current !== headerImg || prevUpdatedRef.current !== game.updated_at) {
      prevImgRef.current = headerImg;
      prevUpdatedRef.current = game.updated_at;
      setImgState("loading");
      setFailedSs(new Set());
      setLoadedSs(new Set());
    }
  }, [headerImg, game.updated_at]);
  ...
  // Valid screenshot indices (exclude failed loads)
  ...
  // Slideshow: runs on hover (always) or global toggle, only when visible
  useEffect(() => {
    const active = ((slideshow && visible && pageFocused) || hovered);
    const vi = validRef.current;
    if (!active || vi.length === 0) {
      setSsIdx(-1);
      return;
    }
  ...
  // Color tint for card background
  ...
  return (
    <div
      ...
      className={`bg-surface rounded-lg overflow-hidden border-2 cursor-pointer transition-all hover:border-accent/50 ${
        selected ? "border-accent ring-2 ring-accent/40 shadow-[0_0_12px_rgba(99,102,241,0.3)]" : "border-border/50"
      }`}
    ...
    >
      ...
      <div className="relative aspect-[460/215] bg-surface2 overflow-hidden">
        {headerImg ? (
          <>
            ...
            <img
              key={`${headerImg}_${game.updated_at}`}
              src={headerImg}
              alt={game.name}
              className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300 ${
                imgState === "loaded" ? (ssIdx >= 0 && loadedSs.has(ssIdx) ? "opacity-0" : "opacity-100") : "opacity-0"
              }`}
              loading="lazy"
      ...
      <div className="p-2.5">
        <div className="flex items-center gap-1">
          <h3 className="text-xs font-medium truncate flex-1" title={game.name}>
            {game.name}
          </h3>
          {game.review_sentiment && (
            <span className="text-[8px] shrink-0" style={{
              color: scoreColor(primaryScore, tintColors || null),
        ...
        <div className="flex items-center gap-1">
          ...
          {primaryScore > 0 && (() => {
            ...
            return secondary > 0 ? (
              <span className="text-[8px] text-muted/60 shrink-0">{secLabel}</span>
            ) : null;
          })()}
        </div>

        {/* Custom tags — grouped by L0 tag with nested subtags */}
                  ...
                  {g.subs.some((s) => s.subtag_name) && (
                    <>
                      ...
                      {g.subs.filter((s) => s.subtag_name).map((s) => (
                        <span key={s.id}
                          className="px-0.5 rounded text-[8px] cursor-pointer hover:ring-1 hover:ring-current"
                          style={{
                            backgroundColor: s.subtag_type === "meta" ? "#f59e0b18" : "#818cf818",
                            color: s.subtag_type === "meta" ? "#f59e0b" : "#818cf8",
                          }}
                          title="Click=include, Right-click=exclude"
              ...
              <span className="text-[9px] text-muted truncate flex-1">
                {genres.map((g, i) => (
                  <span key={`${g}-${i}`}>
                    {i > 0 && " · "}
                    <span
                      className="cursor-pointer hover:text-teal-400 hover:underline"
        ...
        {(() => {
          ...
          return (
            <div className="flex flex-wrap gap-0.5 mt-1">
              {show.map((t, i) => (
                <span key={`${t}-${i}`}
                  className="px-1 py-px rounded text-[8px] bg-white/5 text-muted/80 cursor-pointer hover:bg-white/10 hover:text-indigo-300"
