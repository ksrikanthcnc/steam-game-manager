...
function ScreenshotCell({ game, imgH, ssCount, slideshow, slideDelay, pageFocused }: { game: GameWithTags; imgH: number; ssCount: number; slideshow: boolean; slideDelay: number; pageFocused: boolean }) {
  const aid = game.steam_appid || `manual_${game.id}`;
  const total = game.total_screenshots || 0;
  ...
  return (
    <div className="flex gap-0.5 overflow-hidden items-center">
      {Array.from({ length: show }, (_, slot) => {
        ...
        return (
          <img key={slot} src={`/api/assets/${aid}/ss_${idx}.jpg`} alt=""
...
function ThumbCell({ game, imgH, slideshow, slideDelay, pageFocused, ssColVisible }: { game: GameWithTags; imgH: number; slideshow: boolean; slideDelay: number; pageFocused: boolean; ssColVisible: boolean }) {
  ...
  const src = tick > 0 ? `/api/assets/${aid}/ss_${valid[(tick % valid.length)]}.jpg` : `/api/assets/${aid}/header.jpg`;
  return (
      ...
      return (
        <div className="flex gap-2">
          <button onClick={(e) => { e.stopPropagation(); onEdit?.(game); }}
            className="text-muted hover:text-foreground text-xs">Edit</button>
          <button onClick={(e) => { e.stopPropagation(); if (confirm(`Delete "${game.name}"?`)) onDelete(game.id); }}
            className="text-muted hover:text-danger text-xs">Del</button>
        </div>
      );
    case "labels":
      return (
